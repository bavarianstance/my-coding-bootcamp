...
// function combinedArtistSearch() {
//   connection.query("SELECT * FROM top5000 INNER JOIN top3000 ON top5000.artist = top3000.artist",
// function(err,res){
//   if (err){
//     throw err;
//   }
// for (var i = 0; i < res.length; i++) {
//   if (res[i].artist === "Celine Dion") {
//     console.log(res[i]);
//   }
// } 
//   });
...
function multiSearch() {
        ...
        connection.query(
            "SELECT * FROM top5000 INNER JOIN top3000 ON top5000.artist = top3000.artist AND top5000.year = top3000.year AND top5000.artist = '" + artistName + "'",
            function(err, res) {
                if (err) throw (err);
                
                for (var i=0; i<res.length; i++) {
                    console.log("Artist: " + res[i].artist + " || " + "Album: " + res[i].album + " || " + "Song: " + res[i].song + " || " + "Year: " + res[i].year);
                }
            }
        )
        connection.end();
